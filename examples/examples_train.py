...
MODEL_OPTIONS = config.model_parameters
...
def start_train_setting():
    ...
    MODEL_OPTIONS['model_class'] = model_classes[config.model_parameters['model_name']]
    MODEL_OPTIONS['dataset_file'] = dataset_dict
    MODEL_OPTIONS['inputs_cols'] = input_colses[config.model_parameters['model_name']]
    MODEL_OPTIONS['initializer'] = initializers[config.model_parameters['initializer']]
    MODEL_OPTIONS['optimizer'] = optimizers[config.model_parameters['optimizer']]
    MODEL_OPTIONS['device'] = torch.device('cuda' if torch.cuda.is_available() else 'cpu') if config.model_parameters['device'] is None else torch.device(config.model_parameters['device'])

    log_file = '{}-{}.log'.format(config.model_parameters['model_name'], strftime("%y%m%d-%H%M", localtime()))
    logger.addHandler(logging.FileHandler(log_file))

    opt = Namespace(**MODEL_OPTIONS)
    ins = Instructor(opt)
    ins.run()
